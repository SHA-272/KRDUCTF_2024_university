...
def challenge():
    ...
    if role_cookie is None:
        response = make_response(html_template.format(background_url=default_background, message="Добро пожаловать!"))
    ...
    if not referrer or 'example.com' not in referrer:
        message = 'Кажется, вы пришли не с <a href="https://example.com">example.com</a>'
        return make_response(html_template.format(background_url=referrer_background, message=message))

    # Проверка User-Agent
    user_agent = request.headers.get('User-Agent', '')
    if user_agent != 'venator':
        message = 'Ваш браузер не: venator'
        return make_response(html_template.format(background_url=user_agent_background, message=message))

    # Дальнейшая логика обработки запроса, начиная с проверки куки...
    if role_cookie != 'YWRtaW4=':  # 'admin' в Base64
        message = 'Неверные куки.'
        return make_response(html_template.format(background_url=wrong_cookie_background, message=message))

    # Проверка заголовка Admin
    is_admin = request.headers.get('Admin') == 'true'
    if not is_admin:
        message = 'Admin: false.'
        return make_response(html_template.format(background_url=admin_false_background, message=message))

    # Проверка метода запроса
    if request.method == 'GET':
        message = 'Неверный метод запроса.  '
        return make_response(html_template.format(background_url=wrong_method_background, message=message))
    elif request.method == 'POST':
        message = 'Поздравляем, вы получили флаг: krdu{132_POST_POST_231}. '
        return make_response(html_template.format(background_url=success_background, message=message))
